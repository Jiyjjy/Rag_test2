...
def main():
    ...
    if 'messages' not in st.session_state:
        st.session_state['messages'] = [{"role": "assistant",
                                         "content": "안녕하세요! 주어진문서에 대해궁금하신 것이 있으면 언제든 물어봐주세요"}]
    
    def format_docs(docs):
        return "\n\n".join(doc.page_content for doc in docs)
    
    RAG_PROMPT_TEMPLATE = """당신은 동서울대학교 컴퓨터소프트웨어과 한내 AI 입니다.
                            검샘된 문맥을 사용하여 질문에 맞는 답변을 30단어 이내로하세요
                            답을 모른다면 모른다고 답변하세요.
                            Question: {question}
                            Context: {context}
                            Answer:"""
    ...
    if user_input := st.chat_input("메세지를 입력해 주세요"):
        ...
        with st.chat_message("assistant"):
            ...
            llm = RemoteRunnable("https://coral-tops-piranha.ngrok-free.app/llm/")
